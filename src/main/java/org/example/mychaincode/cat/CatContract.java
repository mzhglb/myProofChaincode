...
//        queryJson.put("use_index", new JSONArray("[\"_design/indexNameColorDoc\", \"indexNameColor\"]"));
//        String query = queryJson.toString();
    ...
    /**
     * 获取数据的哈希值，然后进行比较或验证操作。在不泄露数据具体内容的情况下，验证数据是否被篡改。
     * collection：这是一个字符串类型的参数，表示私有数据集合的名称。在 Hyperledger Fabric 中，私有数据集合是一种机制，用于在特定的组织之间共享数据，而其他组织无法访问这些数据。不同的私有数据集合可以有不同的名称，通过这个名称可以指定要操作的私有数据集合。
     * @param ctx
     * @param collection
     * @param key
     * @return
